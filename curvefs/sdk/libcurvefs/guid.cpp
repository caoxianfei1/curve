...
Mapping Mapping::newMapping(std::string salt) {
    Mapping m(salt);
...
std::string Mapping::lookupUserID(uint32_t id) {
    auto it = userIDs.find(id);
    if (it != userIDs.end()) {
        return it->second;
    }
    ...
    if (pw == nullptr) {
        ...
        strcpy(name, std::to_string(id).c_str()); 
        pw->pw_name = name;
        pw->pw_uid = id;
    }
...
std::string Mapping::lookupGroupID(uint32_t id) {
    auto it = groupIDs.find(id);
    if (it != groupIDs.end()) {
        return it->second;
    } 
